...
*/
let me={name:"hena",surname:"aslam",age:25}
console.log(me)
/* EXERCISE E
...
let nodig=onlyLetters("I have 4 dogs and 23 cats")
console.log(nodig)
...
const howManyDays=function(date){
    ...
    return Math.round((today-givenDate)/(24*60*60*1000))
}
let days=howManyDays("2 july 1997")
console.log(days)
...
function isTodayMyBirthday(date){
    ...
    let myBirthday=new Date(("01/01/1996"));
...
const oldestMovie=function(){
    let abigval=9999999999
    let moviename
    for(i=0;i<movies.length;i++){
        let movie=movies[i]
        if(parseInt(movie.Year)<abigval){
            abigval=movie.Year
...
const searchAndDivide=function(str){
    let res={match:[],unmatch:[]}
    for(i=0;i<movies.length;i++){
        let movie=movies[i]
        if(movie.Title.includes(str)){
            res.match.push(movie)
        }
        else{
            res.unmatch.push(movie)
        }
